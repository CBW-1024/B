...
- (void)viewDidLoad {
    ...
    // 现在用 navigationItem.standardAppearance 按 VC 隔离，使用 configureWithDefaultBackground 让当前页导航栏为半透明(带模糊，与微信原生一致)，
    // 不手动渲染颜色。pop 后微信其他页自动恢复原生外观。微信 8.0.76 最低 iOS 13，无需兼容旧系统。
